...
for k=1:meas.K
    if ~isempty(est.X{k})
        P= est.X{k}([1 3],:);
        hline2= line(k*ones(size(est.X{k},2),1),P(1,:),'LineStyle','none','Marker','o','Markersize',8,'Color',0*ones(1,3));
    end
end
...
for k=1:meas.K
    if ~isempty(est.X{k}),
        P= est.X{k}([1 3],:);
        yhline2= line(k*ones(size(est.X{k},2),1),P(2,:),'LineStyle','none','Marker','o','Markersize',8,'Color',0*ones(1,3));
    end
end

subplot(211); xlabel('Time','Markersize',5); ylabel('x-coordinate (m)','Markersize',5);
...
subplot(212); xlabel('Time','Markersize',5); ylabel('y-coordinate (m)','Markersize',5);
...
subplot(3,1,1); plot(1:meas.K,ospa_vals(:,1),'k'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Dist','Markersize',5);
subplot(3,1,2); plot(1:meas.K,ospa_vals(:,2),'k'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Loc','Markersize',5);
subplot(3,1,3); plot(1:meas.K,ospa_vals(:,3),'k'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Card','Markersize',5);
xlabel('Time','Markersize',5);

%plot cardinality
figure; cardinality= gcf; 
subplot(2,1,1); box on; hold on;
stairs(1:meas.K,truth.N,'k'); 
plot(1:meas.K,est.N,'ko','Markersize',5);
