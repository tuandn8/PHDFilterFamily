...
for k=1:meas.K
    if ~isempty(est.X{k})
        P= est.X{k}([1 3],:);
        hline2= line(k*ones(size(est.X{k},2),1),P(1,:),'LineStyle','none','Marker','o','Markersize',3,'Color','b');
    end
end
...
for k=1:meas.K
    if ~isempty(est.X{k}),
        P= est.X{k}([1 3],:);
        yhline2= line(k*ones(size(est.X{k},2),1),P(2,:),'LineStyle','none','Marker','o','Markersize',3,'Color','b');
    end
end
...
subplot(3,1,1); plot(1:meas.K,ospa_vals(:,1),'b'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Dist');
subplot(3,1,2); plot(1:meas.K,ospa_vals(:,2),'b'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Loc');
subplot(3,1,3); plot(1:meas.K,ospa_vals(:,3),'b'); grid on; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Card');
xlabel('Time');
