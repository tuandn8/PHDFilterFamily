...
model.sensors = 2;                              % number of sensors
model.S= zeros(2, model.sensors);               % sensors location
model.S(:,1) = [-500 ; 0];
model.S(:,2) = [500 ; 0];
model.rd = 1000;
model.R_D=model.rd*model.rd*diag([1 1]);
model.det_RD = det(model.R_D);
model.inv_RD = inv(model.R_D);
