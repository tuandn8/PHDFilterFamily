function est = run_msphd_filter(model,meas)
...
for k=1:meas.K
    ...
    for s= 1 : model.sensors
        %---gating
        if filter.gate_flag
            meas.Z{k,s}= gate_meas_gms(meas.Z{k,s},filter.gamma,model,m_predict,P_predict);
        end
        ...
        if m~=0
            %m detection terms
            [qz_temp,m_temp,P_temp] = kalman_update_multiple(meas.Z{k,s},model,m_predict,P_predict);
            for ell=1:m
