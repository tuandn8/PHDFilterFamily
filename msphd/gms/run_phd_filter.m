...
for k=1:meas.K
    ...
    %---gating
    if filter.gate_flag
        meas.Z{k,1}= gate_meas_gms(meas.Z{k,1},filter.gamma,model,m_predict,P_predict);        
    end
    ...
    if m~=0
        %m detection terms 
        [qz_temp,m_temp,P_temp] = kalman_update_multiple(meas.Z{k,1},model,m_predict,P_predict);
        for ell=1:m
